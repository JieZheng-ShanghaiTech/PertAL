...
class active_learning(Strategy):
    def __init__(self, dataset, net, selection_method, base_kernel, kernel_transforms, device, 
                 ...
                 add_ctrl = False, prior_feat_list = None, gene_hvg_idx = None, lamb = None,strategy_name=None,dataset_name=None,
                 llm_name=None,gradient_weight=None):
        super(active_learning, self).__init__(dataset, net)
    ...
    def query(self, n, save_kernel = False, save_name = None, valid_perts = None, round = None,llm_weight=None):
        strategy = self
        ...
        if self.strategy_name in ['PertAL']:
                
                if self.strategy_name=='PertAL':
                    grad=strategy.get_gradient_new(train_data)
                else:   
                    grad=strategy.get_gradient(train_data)


                pert2grad=dict(pd.DataFrame(zip(grad['pert_cat'], grad['gradients'])).groupby(0)[1].agg(agg_fct))
                pert2score={}
                K = int(grad['gradients'].shape[1]*self.gradient_weight)
                for pert, grad_vec in pert2grad.items():
                    abs_grad = np.abs(grad_vec)
                    top_k_indices = np.argsort(abs_grad)[-K:]
                    top_k_sum = abs_grad[top_k_indices].sum()
                    pert2score[pert] = top_k_sum
        ...
        if self.prior_kernel_list:
            ...
            gradients=np.stack([pert2score[i] for i in pert_list])
            ...
            train_gold = self.train_gold[reindex_train][:, reindex_train].squeeze()
            ...
            bs = BatchSelectorImpl([self.net], {'train': train_data, 'pool': pool_data}, 0, train_gold = train_gold, prior_kernel_list = prior_kernel_list_reindex, 
                        ...
                        lamb = self.lamb, round = round,grad_scores=grad_scores,llm_scores=llm_scores,llm_weight=llm_weight)
            new_idxs, _ = bs.select(selection_method=self.selection_method + '_priorgradllm', sel_with_train=self.sel_with_train,
