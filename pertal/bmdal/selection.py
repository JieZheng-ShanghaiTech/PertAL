...
class MaxDiagSelectionMethod(SelectionMethod):
    ...
    def select_score(self, batch_size: int) -> torch.Tensor:
        diag = self.pool_features.get_kernel_matrix_diag()
        return diag
...
class  MaxDistSelectionMethodwithPriorGrad(IterativeSelectionMethod):
    """
    Implements the MaxDist selection method for Batch Active Learning,
    with optional gradient-based scores combined with distance scores.
    """
    def __init__(
        self,
        ...
        **config
        ...
        dtype = prior.dtype if prior is not None else torch.float32

        self.min_sq_dists = torch.full(
        ...
        if grad_scores is not None:
            if isinstance(grad_scores, np.ndarray):
                grad_scores = torch.from_numpy(grad_scores)

            grad_scores = grad_scores.to(device=device, dtype=dtype)

            assert grad_scores.shape[0] == self.pool_features.get_n_samples(), \
                f"grad_scores length {grad_scores.shape[0]} must be equal to pool size {self.pool_features.get_n_samples()}"

            mean = grad_scores.mean()
            std = grad_scores.std()
            if std > 0:
                grad_scores = (grad_scores - mean) / (std + 1e-8)

            self.grad_scores = grad_scores
        else:
    ...
    def get_scores(self) -> torch.Tensor:
        ...
        div_scores = self.min_sq_dists

        div_mean = div_scores.mean()
        div_std = div_scores.std()
        div_scores = (div_scores - div_mean) / (div_std + 1e-8)

        return self.alpha * div_scores + self.beta * self.grad_scores
...
class  MaxScoreSelectionMethodwithPriorGradLLM(IterativeSelectionMethod):
    """
    Implements the MaxScore selection method for Batch Active Learning,
    with optional gradient-based scores, llm proir score, combined with diversity scores.
    """
    def __init__(
        self,
        pool_features: Features,
        ...
        **config
        ...
        if llm_scores is not None:
            if isinstance(llm_scores, np.ndarray):
                llm_scores = torch.from_numpy(llm_scores)

            llm_scores = llm_scores.to(device=device, dtype=dtype)

            assert llm_scores.shape[0] == self.pool_features.get_n_samples(), \
                f"llm_scores length {llm_scores.shape[0]} must be equal to pool size {self.pool_features.get_n_samples()}"

            mean = llm_scores.mean()
            std = llm_scores.std()
            if std > 0:
                llm_scores = (llm_scores - mean) / (std + 1e-8)
    ...
    def get_scores(self) -> torch.Tensor:
        ...
        div_std = div_scores.std()
        div_scores = (div_scores - div_mean) / (div_std + 1e-8)
        # if self.grad_scores is None:
        #     return div_scores

        return self.alpha * div_scores + self.beta * self.grad_scores + self.gamma * self.llm_scores
