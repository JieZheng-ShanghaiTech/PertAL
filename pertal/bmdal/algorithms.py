...
class BatchSelectorImpl:
    ...
    def __init__(self, models: List[nn.Module], data: Dict[str, FeatureData], y_train: Optional[torch.Tensor], 
                 ...
                 normalize_mode = 'diag', valid_perts = None, lamb = None, round = None,grad_scores=None,llm_scores=None,llm_weight=None):
        """
    ...
    def select(self, base_kernel: str, kernel_transforms: List[Tuple[str, List]], selection_method: str,
        ...
        #----------------PertAL----------------------#
        elif selection_method == 'maxdist_priorgradllm':
            k_agg = torch.tensor(k_agg).to(self.device)
            alg = MaxScoreSelectionMethodwithPriorGradLLM(self.features['pool'], self.features['train'],
                                                       prior = k_agg,grad_scores=self.grad_scores,llm_scores=self.llm_scores,gamma=self.llm_weight, **config)
        ...
        if selection_method not in ['maxdist_prior_grad']:
            ...
            batch_score=alg.select_score(batch_size)
            return batch_score
