...
class GEARS:
    def __init__(self, pert_data, 
        ...
        if self.weight_bias_track:
            import swanlab 
            if not swanlab.run: 
              swanlab.init(project=proj_name, name=exp_name)  
            self.wandb = swanlab
        else:
            self.wandb = None
    ...
    def get_gradient(self,loader):
        ...
        for step, batch in enumerate(loader):
            ...
            # batch_gradients = batch.x.grad.cpu().numpy()  
            # gradients.append(batch_gradients) 
            for i in range(batch.x.size(0)):  
                sample_grad = batch.x.grad[i].cpu().numpy()  
        ...
        results['gradients'] = np.array(gradients).reshape(-1, 1000)

        return results
    

    def get_gradient_new(self,loader):
        self.model.eval()
        pert_cat = []  
        gradients = []  
        results = {}
    ...
    def get_gradient_new(self,loader):
        ...
        for step, batch in enumerate(loader):
            ...
            batch.x.requires_grad_()
            pred=self.model(batch)
            sensitivity_loss = torch.norm(pred, p=2, dim=1).sum()
            
            sensitivity_loss.backward()
            self.model.zero_grad()
        
            for i in range(batch.x.size(0)):  
                sample_grad = batch.x.grad[i].abs().cpu().numpy()  
                gradients.append(sample_grad) 
