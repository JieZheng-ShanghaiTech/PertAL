...
def get_latent_emb(loader, model, device, return_h):
    model.eval()
    model.to(device)
    pert_cat = []
    hid_cat = []
    ...
    for itr, batch in enumerate(tqdm(loader)):
        ...
        with torch.no_grad():
            out = model(batch, return_h)
            hid_cat.extend(out.reshape(len(batch), -1).cpu())

    results['pert_cat'] = np.array(pert_cat)
    hid_cat = torch.stack(hid_cat)
    results['latent_feat']= hid_cat.detach().cpu().numpy()
...
def evaluate_mean_ctrl(loader, model, device, ctrl_mean):
    model.eval()
    model.to(device)
    pert_cat = []
    pred = []
    ...
    for itr, batch in enumerate(tqdm(loader)):
        ...
        with torch.no_grad():
            out = model(batch, 'diff_effect')
            #print('Out...')
            #print(out)
            p = out.reshape(len(batch), -1).cpu() + ctrl_mean
            #print('P...')
            #print(p)
            pred.extend(p)
            t = batch.y
...
class eval_analysis_fast:
    ...
    def __init__(self, adata):
        ...
        pert_list = np.array(list(condition2mean_expression.keys()))
        mean_expression = np.array(list(condition2mean_expression.values())).reshape(len(adata.obs.condition.unique()), adata.X.toarray().shape[1])
        self.ctrl = mean_expression[np.where(pert_list == 'ctrl')[0]]
        '''
    ...
    def per_perturb_analysis(self, pert, test_res):
        ...
        # 'pearson_delta de'
        val = pearsonr(test_res['pred'][pert_idx].mean(0)[de_idx]- ctrl[0][de_idx], test_res['truth'][pert_idx].mean(0)[de_idx]-ctrl[0][de_idx])[0]
        if np.isnan(val):
            val = 0
        res['pearson_delta_top20_de_non_dropout'] = val
        ...
        # 'mse_non_dropout'
        val = mse(test_res['pred'][pert_idx].mean(0), test_res['truth'][pert_idx].mean(0))
        if np.isnan(val):
            val = 0
        res['mse_non_dropout'] = val

         # 'mse_4_non_dropout'
        val = np.sum((test_res['pred'][pert_idx].mean(0) - test_res['truth'][pert_idx].mean(0))**4)/len(test_res['pred'][pert_idx].mean(0))
        if np.isnan(val):
            val = 0
        res['mse_4_non_dropout'] = val

        # 'mse_4_top20_de_non_dropout'
        val = np.sum((test_res['pred'][pert_idx].mean(0)[de_idx] - test_res['truth'][pert_idx].mean(0)[de_idx])**4)/len(de_idx)
        if np.isnan(val):
            val = 0
        res['mse_4_top20_de_non_dropout'] = val
