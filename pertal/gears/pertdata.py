...
from .utils import print_sys, zip_data_download_wrapper, dataverse_download,\
                  filter_pert_in_go, get_genes_from_perts,filter_pert_in_prior
...
class PertData:
    ...
    def set_pert_genes(self):
        ...
        with open(f'{self.data_path}/{self.dataset_name}_kernels/knowledge_kernels_1k/scgpt_blood/pert_list.pkl','rb') as f:
            self.prior_names=pickle.load(f)
    ...
    def load(self, data_name = None, data_path = None):
        ...
        print_sys('These perturbations are not in the prior kernerl and their '
                  'perturbation can thus not be predicted')
        not_in_prior_pert = np.array(self.adata.obs[
                                  self.adata.obs.condition.apply(
                                  lambda x:not filter_pert_in_prior(x,
                                        self.prior_names))].condition.unique())
        print_sys(not_in_prior_pert)
        
        filter_prior = self.adata.obs[self.adata.obs.condition.apply(
                              lambda x: filter_pert_in_prior(x, self.prior_names))]
        self.adata = self.adata[filter_prior.index.values, :]
